    ...
    >
      <div className={"flex flex-1 flex-col"}>
        ...
        <div className={"px-2 py-6"}>
          <div className={"flex space-x-2 items-center"}>
            ...
            <h1 className={"font-semibold text-xl"}> {_program?.name}</h1>
          </div>
          ...
          <div className={"flex flex-col space-y-4 mt-3 text-start"}>
            ...
            <span className={"text-md font-medium text-divider/60"}>
              Platform: {_program?.platform ? _program?.platform.name : "--"}
            </span>
            <span className={"text-md font-medium text-divider/60"}>
              Commision type:{" "}
            ...
            <div className="flex items-center gap-1 mt-0">
              {"★★★★☆".split("").map((star, i) => (
                <span key={i} className="text-purple-300 text-xl">
                  {i < Math.floor(getRateValue(_program?.average_ratings))
                    ? "★"
                    : "☆"}
                </span>
              ))}
          ...
          <div className={"flex flex-col space-y-4 py-6 mt-8"}>
            <div className={"flex space-x-6"}>
              <span className={"text-lg font-medium"}>
                {_program?.up_votes ? _program?.up_votes.length : 0} Upvotes
              </span>
              <span className={"text-lg font-medium"}>
      ...
      <div className={"w-full md:max-w-[320px] flex-1 px-6"}>
        ...
        <div className="flex flex-col space-y-6">
          ...
          >
            <div className="p-1 flex flex-col items-center md:items-center">
              <h3 className={"text-lg font-medium"}>{_program?.name}</h3>
              <div className="flex items-center gap-1 mt-0">
                {"★★★★☆".split("").map((star, i) => (
                  <span key={i} className="text-purple-300 text-xl">
                    {i < Math.floor(getRateValue(_program?.average_ratings))
            ...
            <Button
              ...
              variant={"light"}
              onClick={() =>
                router.push(`${_program?.link ? _program?.link : "#"}`)
              }
            >
              Follow
            </Button>
            ...
            {samples.map((sample, i) => (
              <div key={i} className={"p-4"}>
                <div className={"flex flex-col items-start"}>
                  <h3 className={"text-lg font-medium"}>{sample.name}</h3>
                  <span className={"text-sm text-divider/60"}>
                    Platform: {sample.platform ? sample.platform.name : "--"}
                  </span>
                  <span className={"text-sm text-divider/60"}>
                    Commision type:{" "}
                  ...
                  <div className="flex items-center gap-1 mt-0">
                    {"★★★★☆".split("").map((star, i) => (
                      <span key={i} className="text-purple-300 text-xl">
                        {i < Math.floor(getRateValue(sample.average_ratings))
                          ? "★"
                          : "☆"}
                      </span>
                    ))}
